...
nos demais casos)*/
int hash2(int chave, int tamanho_arquivo) {
...
void insere(vector<int>& tabela_hash, int tamanho_arquivo, int& total_chaves_inseridas, int& total_acessos) {
    ...
    while(total_chaves_inseridas < tamanho_arquivo) { // se o total de chaves alcançar o número de registros disponíveis encerra o loop
        ...
        int h2 = hash2(chave, tamanho_arquivo); // chamada para calcular o h2
        ...
        if (tabela_hash[h1] == -1) { // caso em que, usando h1, a posição calculada está disponível
            ...
            int i = 1;
            int nova_posicao = (h1 + i*h2) % tamanho_arquivo; // calcula a nova posição usando o h2 
            total_acessos++; // incrementa o total de acessos
...
int main() {
    ...
    vector<int> tabela_hash = criar_tabela_hash(tamanho_arquivo); // chama a função para criar a tabela hash

    insere(tabela_hash, tamanho_arquivo, total_chaves_inseridas, total_acessos); // chama função para inserção das chaves
    ...
    saida(tabela_hash, tamanho_arquivo, total_acessos, total_chaves_inseridas); // chama função para produzir a saída
